...
extern "C" {
	...
	#define ABS_SUB(a,b)							( (a)>(b)?((a)-(b)):((b)-(a)) )										//---两个数差的绝对值
	#define SWAP_16BITS(val)						( ((val&0x00FF)<<8)|((val&0xFF00)>>8) )                             // 高低字节互换
																														// 高低16bit互换
	#define SWAP_32BITS(val)						(((val&0x000000FF)<<24)|((val&0x0000FF00)<<8)|((val&0x00FF0000)>>8)|((val&0xFF000000)>>24))                      
...
#if defined(MCU_USE_STM32F0)||defined(MCU_USE_STM32F1)|| \
	defined(MCU_USE_STM32F2)||defined(MCU_USE_STM32F3)|| \
	defined(MCU_USE_STM32F4)
	//===选择STM32型号
	#define MCU_USE_STM32	
#elif defined(MCU_USE_RF78_R7F0)
	//===选择瑞萨型号
	#define MCU_USE_RL78
#else
	#error "不支持型号的MCU,只支持STM32F0，F1，F2，F3，F4系列!"
#endif	
	//////////////////////////////////////////////////////////////////////////
	//===依据MCU型号，包含不同的头文件
